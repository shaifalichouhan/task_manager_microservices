...
class AuthService:
    ...
    def __init__(self):
        self.base_url = settings.auth_service.url
        self.timeout = settings.auth_service.timeout
        self.retries = settings.auth_service.retries
...
def require_user_id(user_id: int):
    """
    Dependency factory to require specific user ID.
    ...
    """
    async def _require_user_id(current_user: CurrentUser = Depends(get_current_user)):
